...
export async function fetchProducts(): Promise<Product[]> {
    const response = await axios.get<Product[]>("http://localhost:8070/products");
...
export async function createProduct(formData: FormData) {
    ...
    const response = await axios.post("http://localhost:8070/products", formData, config);
    return response.data;
}

// 제품 상세 조회
export async function fetchProductDetail(id:number) : Promise<ProductDetail> {
    const response = await axios.get<ProductDetail>(`http://localhost:8070/products/${id}/with-detail-images`)
    return response.data;
}

// 상세 이미지 조회
export async function fetchDetailImageUrls(id:number) : Promise<DetailImageUrl[]>{
    const response = await axios.get<DetailImageUrl[]>(`http://localhost:8070/products/${id}/detail-images`);
    return response.data;
}
...
export async function deleteDetailImage(id:number){
    await axios.delete(`http://localhost:8070/products/detail-images/${id}`);
};
